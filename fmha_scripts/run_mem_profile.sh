...
#nsys profile -w true -t cuda,nvtx,osrt,cudnn,cublas -s cpu --cuda-memory-usage=true --capture-range=cudaProfilerApi --capture-range-end=stop-shutdown --cudabacktrace all --force-overwrite true -x true -o $script python3 my_test_fmha_new_64.py
#nsys profile -w true -t cuda,nvtx,osrt,cudnn,cublas -s cpu --cuda-memory-usage=true --capture-range=cudaProfilerApi --capture-range-end=stop-shutdown --cudabacktrace all --force-overwrite true -x true -o $script python3 my_test_fmha_new_128.py
nsys profile -w true -t cuda,nvtx,osrt,cudnn,cublas -s cpu --cuda-memory-usage=true --capture-range=cudaProfilerApi --capture-range-end=stop-shutdown --cudabacktrace all --force-overwrite true -x true -o $script python3 my_test_fmha_new_64.py
