...
#
#    #assert(torch.allclose(ctx_ref.float(), ctx.float(), atol=1e-3))
#
...
#
#    labels = torch.randn_like(qkv_vs)

#    dw2 = torch.randn((b, s, h, d), dtype=dtype, device=device).contiguous()
#
#    iters = 50
#    for it in range(iters):
...
#    dqkv2 = dqkv2.permute(0,2,1,3).view(b,s, h,3,d)
    ...
    #assert(torch.allclose(qkv.grad.float(), dqkv2.float(), atol=1e-3))
