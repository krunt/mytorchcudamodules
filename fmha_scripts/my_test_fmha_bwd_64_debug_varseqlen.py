...
def run_test(s, b, h = 16, d = 64):
    ...
    #print('q', dgrad[:,:,:,0,:], dqkv2[:,:,:,0,:].float())
    #print('k', dgrad[:,:,:,1,:], dqkv2[:,:,:,1,:].float())
    #print('v', dgrad[:,:,:,2,:], dqkv2[:,:,:,2,:].float())

    assert(torch.allclose(dgrad, dqkv2.float(), atol=0.25))
#    assert(torch.allclose(dgrad[:,:,:,0,:], dqkv2[:,:,:,0,:].float(), atol=0.25))
#    assert(torch.allclose(dgrad[:,:,:,1,:], dqkv2[:,:,:,1,:].float(), atol=0.25))
#    assert(torch.allclose(dgrad[:,:,:,2,:], dqkv2[:,:,:,2,:].float(), atol=0.25))


torch.cuda.cudart().cudaProfilerStart()
run_test(4*384, 12, h=8, d=64)
#run_test(4*384, 4)
#run_test(512, 128)
#run_test(1024, 32)
